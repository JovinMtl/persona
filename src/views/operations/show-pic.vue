<template>
    <div class="centered welcomeText bigWidth">
        <div class="mt-20 fw-b">
            Nsanzumukiza Thierry Jovin
        </div> 
       
        <div class="m-20 c-b ff-o">
            I'm Thierry, a Full Stack Developer based in <span style="font-weight: 700;">Burundi</span>
            
            with over 2 years' experience in programming. I'm passionate about creating secure, scalable web solutions with Python, Django, Vue.js, and exploring React for my future projects.
        </div>
    </div>
</template>
